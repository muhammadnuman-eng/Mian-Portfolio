...
const WorksSection = () => {
  ...
  return (
    <section id="works" ref={sectionRef} className="py-20 golden-gradient relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-10 right-20 w-40 h-40 hexagon border border-navy/20"></div>
        <div className="absolute bottom-10 left-20 w-32 h-32 hexagon border border-navy/20"></div>
        <div className="absolute top-1/2 right-1/3 w-24 h-24 hexagon border border-navy/20"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16 relative z-10">
        ...
        <div className={`mb-16 fade-in-up ${isVisible ? 'visible' : ''}`}>
          <div className="text-center space-y-4">
            <div className="flex items-center justify-center space-x-4 mb-6">
              <div className="w-8 h-8 hexagon bg-gradient-to-br from-navy to-navy-light animate-scale-in shadow-lg"></div>
              <h2 className="text-2xl font-bold text-navy tracking-wide">Featured Works</h2>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {featuredProjects.map((project, index) => (
            <div
              key={project.id}
              className={`group bg-navy rounded-2xl overflow-hidden hover:scale-105 hover:-translate-y-3 transition-all duration-700 cursor-pointer fade-in-up ${isVisible ? 'visible' : ''
        ...
        <div className={`text-center fade-in-up ${isVisible ? 'visible' : ''}`} style={{ animationDelay: '0.8s' }}>
          <Button
            ...
            className="group bg-navy text-golden hover:bg-navy-light hover:scale-105 transition-all duration-300 px-10 py-4 text-base font-semibold rounded-full shadow-xl hover:shadow-2xl flex items-center gap-3 mx-auto border border-golden/20 hover:border-golden/40"
          >
